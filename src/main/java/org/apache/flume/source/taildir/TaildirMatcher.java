...
public class TaildirMatcher {
    ...
    private File parentDirpre;
    private Map<String, File> parentDirMap = new ConcurrentHashMap<>();
    ...
    TaildirMatcher(String fileGroup, String filePattern, boolean cachePatternMatching) {
        ...
        //[zhuzhigang] add
        if (this.parentDir.getAbsolutePath().contains("*")) {
            parentDirpre = this.parentDir.getParentFile();
            File[] temfile = this.parentDir.getParentFile().listFiles();
            for (File file : temfile) {
                if (file.isDirectory()) {
                    this.parentDirMap.put(file.getName(), file);
                }
            }
        }

        String regex = f.getName();
        logger.info("[zork]查看 regex:{}", regex);
...
//        Preconditions.checkState(parentDir.exists(), "Directory does not exist: " + parentDir.getAbsolutePath());
        if (parentDirpre != null) {
            Preconditions.checkState(parentDirpre.exists(), "Directory does not exist: " + parentDirpre.getAbsolutePath());
        } else {
            Preconditions.checkState(parentDir.exists(), "Directory does not exist: " + parentDir.getAbsolutePath());
    ...
    List<File> getMatchingFiles() {
        ...
        if (parentDirpre != null) {
            for (File f : parentDirMap.values()) {
                currentParentDirMTime = f.lastModified();
                List<File> result;
    ...
    private List<File> getMatchingFilesNoCache() {
        ...
        if (parentDirpre != null) {
            ...
            for (File paren : parentDirMap.values()) {
                try (DirectoryStream<Path> stream = Files.newDirectoryStream(paren.toPath(), fileFilter)) {
                    for (Path entry : stream) {
                        result.add(entry.toFile());
                    }
                } catch (IOException e) {
                    logger.error("I/O exception occurred while listing parent directory. " +
                            "Files already matched will be returned. " + paren.toPath(), e);
                }
            }
