...
import org.apache.flume.source.taildir.windows.WinFileUtil;
...
public class ReliableTaildirEventReader implements ReliableEventReader {
    ...
    private ReliableTaildirEventReader(Map<String, String> filePaths,
        ...
        for (Entry<String, String> e : filePaths.entrySet()) {
            logger.info("[zork]查看：key:{},value:{}",e.getKey(),e.getValue());
...
//                    logger.info("[zork]查看Header{}",event.getHeaders());
                }
                if (annotateFileName) {
...
//    return inode;
        long inode;
        if (OS_NAME.trim().startsWith("win")) {
